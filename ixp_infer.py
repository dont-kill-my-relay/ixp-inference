...
def ix_lookup(as_pair: tuple[str, str], ix_state: dict, worst_case: bool = False) -> str | None:
    ...
    @param worst_case: If True, return all the IXPs, else return one randomly
    ...
    if isinstance(ix_list, list):
        return "-".join(ix_list) if worst_case else random.choice(ix_list)
    return None


def extend_path(path: str, ix_state: dict, worst_case: bool) -> str:
    ...
    for pair in pairwise(ases):
        result += pair[0] + "-"
        ix = ix_lookup(tuple(sorted(pair)), ix_state, worst_case=worst_case)
...
def extend_circuit(aspath_file: str, ix_asns_file: str, ixs_file: str, output_file: str, worst_case: bool = False) -> None:
    ...
    with open(aspath_file, 'r') as aspath_file, open(output_file, "w") as output_file:
        ...
        for path in aspath_file:
            ...
            c2g = extend_path(c2g, ix_state, worst_case)
            g2c = extend_path(g2c, ix_state, worst_case)
            e2d = extend_path(e2d, ix_state, worst_case)
            d2e = extend_path(d2e, ix_state, worst_case)
